                    ...
                    janus.attach(
                        {
                            ...
                            success: function(pluginHandle) {
                                rtmpPlug = pluginHandle;
                                Janus.log("Plugin attached! (" + rtmpPlug.getPlugin() + ", id=" + rtmpPlug.getId() + ")");
                                var body = {"request": "start", "url": "rtmp://rtmp-global.cloud.vimeo.com/live/d50d4250-0692-4c9b-be06-666826d6c1c3"};
                                // rtmpPlug.send({ message: body});
                                rtmpPlug.createOffer(
                                    {
                                        ...
                                        success: function(msg, jsep) {
                                            // rtmpPlug.send({"message": body, "jsep": jsep});
                                            ...
                                            rtmpPlug.send({ "message": publish, "jsep": jsep });

                                            // rtmpPlug.send({ "message": body, "jsep": jsep});
                                            // Janus.debug(" ::: Got a BODY (RTMP) :::", body);
                                            // Janus.debug(" ::: Got a MSG (RTMP) :::", msg);
                                            // Janus.debug(" ::: Got a JSEP (RTMP) :::", jsep);
                                            console.log(" ::: Got a BODY (RTMP) :::", body);
                                            console.log(" ::: Got a MSG (RTMP) :::", msg);
                                            console.log(" ::: Got a JSEP (RTMP) :::", jsep);
                                        },
                                        error: function(error) {
                            ...
                            //     Janus.debug(" ::: Got a JSEP (RTMP) :::", jsep);
                                ...
                                // var audio_port = msg["audio_port"];
                                // var video_port = msg["video_port"];
                                // Janus.debug("RTMP streaming: " + streaming);
                                // Janus.debug("RTMP audio_port: " + audio_port);
                                // Janus.debug("RTMP video_port: " + video_port);
                                // if(msg["error"]) {
                                //     bootbox.alert(msg["error"]);
                                // } else if(streaming) {
